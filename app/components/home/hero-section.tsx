...
export function HeroSection() {
  ...
  useEffect(() => {
    // Set origin dynamically to avoid localhost issues
  ...
  return (
    <section className="relative h-[27vh] lg:h-screen w-full overflow-hidden" style={{ position: 'relative' }}>
      <div className="absolute inset-0 z-0">
        {!hasError ? (
          <iframe
            className="absolute top-1/2 left-1/2 w-[130%] h-[110%] -translate-x-1/2 -translate-y-1/2"
            src={`https://www.youtube-nocookie.com/embed/rHgJADRsqFs?version=3&enablejsapi=1&html5=1&hd=1&wmode=opaque&showinfo=0&rel=0&controls=0&playsinline=1&autoplay=1&mute=1&loop=1&playlist=rHgJADRsqFs&disablekb=1&fs=0&modestbranding=1&iv_load_policy=3&cc_load_policy=0&start=0&end=0${origin ? `&origin=${encodeURIComponent(origin)}` : ""}`}
          ...
          // Fallback background image if iframe fails
          <div 
