...
const cityVariants = [
  {
    name: "New Honda City",
    price: "Rp 402.000.000",
    image: "/new-honda-city/spesifikasi-New Honda City.webp",
    specs: [
      "1.5L DOHC i-VTEC Engine 121PS",
      "Honda SENSING",
      '16" Dashing Alloy Wheel Design',
      "LED Headlight with LED DRL",
      "LED Fog Light",
      "Auto Headlight",
      '4.2" Interactive TFT Meter Cluster',
      '8" Advanced Capacitive Touchscreen Display Audio',
      "8-Speakers",
      "6 Airbags",
      "Paddle Shift",
      "Remote Engine Start",
...
export function SpecificationSection() {
  ...
  const currentColorIndex = cityColors.findIndex((color) => color.name === selectedColor.name)
  const canGoPrev = currentColorIndex > 0
  const canGoNext = currentColorIndex < cityColors.length - 1
  
  const handlePrevColor = () => {
    if (canGoPrev) {
      setSelectedColor(cityColors[currentColorIndex - 1])
    }
  }
    ...
    <section className="py-16 lg:py-24 bg-white">
      <div className="container mx-auto px-4">
        <div className="max-w-full overflow-hidden relative">
          ...
          >
            <CarouselContent>
              {cityVariants.map((variant, index) => (
          ...
          <DialogContent className="w-screen h-screen max-w-full max-h-full bg-white p-0 m-0 flex flex-col">
            <DialogHeader className="flex-shrink-0 p-4 border-b border-honda-gray-light relative">
              ...
              <DialogDescription className="sr-only">
                Color selection modal for New Honda City
              </DialogDescription>
</DialogHeader>
